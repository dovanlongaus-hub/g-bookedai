...
const WELCOME_MESSAGES: Record<string, string> = {
  en: "Hi! I'm BookedAI's assistant. I can help you learn about our AI Revenue Engine platform, features, pricing, or answer any questions. How can I help?",
  vi: "Xin chao! Toi la tro ly AI cua BookedAI. Toi co the giup ban tim hieu ve nen tang AI Revenue Engine, tinh nang, gia ca, hoac tra loi bat ky cau hoi nao. Ban can gi?",
};
...
export function ChatWidget() {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: lang === 'vi' ? 'Xin loi, vui long thu lai.' : 'Sorry, please try again.' }]);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        padding: '12px 16px', background: 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(129,140,248,0.1))',
        borderBottom: '1px solid rgba(255,255,255,0.08)',
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#22c55e' }} />
          <span style={{ fontWeight: 600, color: '#fff', fontSize: '0.9rem' }}>BookedAI Assistant</span>
      ...
      {messages.length <= 1 && (
        <div style={{ padding: '0 16px 8px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {(lang === 'vi'
            ? ['Xem tinh nang', 'Bang gia', 'Demo']
            : ['View features', 'Pricing', 'See demo']
          ).map(s => (
      ...
      <form onSubmit={handleSend} style={{
        ...
        <input
          ...
          placeholder={lang === 'vi' ? 'Nhap tin nhan...' : 'Type a message...'}
          style={{
